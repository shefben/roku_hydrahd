...
tools/
  build_zip.bat                One-shot Windows builder � auto-detects
                               LAN IP, bakes it into the channel as the
                               default resolver URL, and emits HydraHD.zip
...
2. Build `HydraHD.zip`. Either use the helper script (Windows):
   ...
   project root. The working tree is never modified, so re-running on
   a clean checkout always produces a fresh build.

   Or, zip manually: `manifest`, `source/`, `components/`, and
   `images/` should all be at the **root** of the zip (don't zip the
   parent directory). Skip `resolver/` and `tools/` � the Roku doesn't
   need them.
...
If you built the zip with `tools\build_zip.bat`, that URL is already
the default � you don't need to type it in Settings unless you want to
override it. Hitting **Clear** in Settings drops the override and falls
back to the baked-in URL.
